-- ═══════════════════════════════════════════════════════════
-- PadelZero — V3-02 STREAKS & MULTIPLIERS MIGRATION
-- ═══════════════════════════════════════════════════════════
...
begin
  ...
  if not exists (select 1 from information_schema.columns where table_name='profiles' and column_name='max_streak') then
    alter table public.profiles add column max_streak integer default 0;
  end if;
end $$;

-- 2. Update update_match_elo with streak logic
...
begin
  ...
  foreach pid in array winner_ids loop
    if pid is not null then
      ...
      -- Calculate Streak Multiplier (starts AFTER this win is counted, so 2 existing + this win = 3)
      p_multiplier := 1.0;
