-- Security hardening based on competitive analysis and security audit
-- Fixes:
-- 1. record_liga_match: verify all 4 players are liga members
-- 2. record_liga_match: duplicate player guard
-- 3. record_liga_match: score bounds enforcement
-- 4. record_liga_match: rate limiting
-- 5. record_liga_match: guest player restriction
-- 6. Prevent direct ELO manipulation via profile UPDATE
-- 7. Restrict matches INSERT to participants only

-- ============================================================
-- 1. Replace record_liga_match with hardened version
-- ============================================================
...
BEGIN
  ...
  -- 1. Validate caller is an active member of the liga
  IF NOT EXISTS (
  ...
  -- 2. Duplicate player guard: no player can appear twice
  ...
  -- 3. Verify ALL 4 players are active members of this liga
  IF (
  ...
  -- 4. Guest player restriction: block guest players from ranked matches
  IF EXISTS (
    SELECT 1 FROM profiles
    WHERE id IN (p_team_a_player1, p_team_a_player2, p_team_b_player1, p_team_b_player2)
      AND is_guest = true
  ) THEN
    -- Check if liga allows guests (via a convention: if liga has no restriction, allow)
    -- For now, we skip ELO for guest matches by checking later
    NULL; -- Allow the match but flag below
  END IF;
  ...
  -- Enforce liga max score if set
  SELECT COALESCE(
    (schedule->>'max_score')::integer,
    4
  ...
  -- 6. Rate limiting: max 20 matches per user per hour
  SELECT count(*) INTO v_recent_count FROM liga_matches
  WHERE recorded_by = v_user_id AND created_at > NOW() - INTERVAL '1 hour';
  IF v_recent_count >= 20 THEN
    RAISE EXCEPTION 'Rate limit: too many matches recorded this hour. Try again later.';
  END IF;

  -- 7. Anti-collusion: max 5 identical matchups per day
  ...
  -- Read current ELO data for all 4 players
  ...
  -- Check if any player is a guest — skip ELO updates for guest matches
  IF EXISTS (
    SELECT 1 FROM profiles
    WHERE id IN (p_team_a_player1, p_team_a_player2, p_team_b_player1, p_team_b_player2)
      AND is_guest = true
  ...
  -- Set bypass flag so the prevent_elo_self_update trigger allows our updates
  ...
  VALUES (
    p_liga_id, LEAST(p_team_a_player1, p_team_a_player2), GREATEST(p_team_a_player1, p_team_a_player2),
  ...
  INSERT INTO liga_pair_stats (liga_id, player1_id, player2_id, matches_played, matches_won, total_score_for, total_score_against)
  VALUES (
    p_liga_id, LEAST(p_team_b_player1, p_team_b_player2), GREATEST(p_team_b_player1, p_team_b_player2),
  ...
  -- Upsert team stats with team ELO
...
-- ============================================================
-- 2. Prevent direct ELO manipulation via profile UPDATE
-- Users can update their display_name, avatar, etc. but NOT elo/stats
-- ============================================================
...
AS $$
BEGIN
  -- Allow updates from SECURITY DEFINER RPCs that set the bypass flag
  -- (e.g., record_liga_match sets app.bypass_elo_guard = 'true')
  IF current_setting('app.bypass_elo_guard', true) = 'true' THEN
    RETURN NEW;
  END IF;

  -- For regular authenticated users via PostgREST, silently prevent
  -- changes to ELO-related and privileged fields
...
DROP TRIGGER IF EXISTS trg_prevent_elo_self_update ON profiles;
CREATE TRIGGER trg_prevent_elo_self_update
  BEFORE UPDATE ON profiles
  FOR EACH ROW
  EXECUTE FUNCTION prevent_elo_self_update();

-- ============================================================
-- 3. Restrict matches INSERT: caller must be a participant
-- (matches table has no created_by column — uses player IDs)
-- ============================================================
DROP POLICY IF EXISTS "auth create match" ON public.matches;
